...
These four are the same four we code on the literature side, so that we can compare what the field writes against what the field does. They are adapted from Arnstein's (1969) ladder of citizen participation, which we use as a shared analytic axis — a way of asking the same questions of everyone — not as a scale that ranks anybody.
...
Where the four framework dimensions come from. Arnstein's ladder is explicitly a ranking; we borrow its questions as a shared axis, not its ordering.
