...
**[Reason for update](#reason-for-update)** — assurance that no data have been observed.

**Class 1 — corrections (8 items).** Errors, omissions and internal contradictions; none changes the design.

- [1.1 Model name in the screening AI-use disclosure](#11-model-name-in-the-screening-ai-use-disclosure)
- [1.2 Agreement statistic for the inductive category coding](#12-agreement-statistic-for-the-inductive-category-coding)
- [1.3 Completing the RAISE citation](#13-completing-the-raise-citation)
- [1.4 Methodological citations for the organizational strand](#14-methodological-citations-for-the-organizational-strand)
- [1.5 Corrections to the frozen seed file](#15-corrections-to-the-frozen-seed-file)
- [1.6 Seed 20 — no DOI, and the registered expansion input is a DOI list](#16-seed-20-no-doi-and-the-registered-expansion-input-is-a-doi-list)
- [1.7 Retraction screening has no registered consequence](#17-retraction-screening-has-no-registered-consequence)
- [1.8 The End date contradicts the attached execution protocol](#18-the-end-date-contradicts-the-attached-execution-protocol)

**Class 2 — design changes (9 items).** Each states what was registered, what replaces it, and why.

- [2.1 The framework dimensions become five](#21-the-framework-dimensions-become-five)
- [2.2 Ecosystem role is removed from the saturation criterion](#22-ecosystem-role-is-removed-from-the-saturation-criterion)
- [2.3 Undetermined values and the saturation counter](#23-undetermined-values-and-the-saturation-counter)
- [2.4 The search validation set is rebuilt so that it can fail](#24-the-search-validation-set-is-rebuilt-so-that-it-can-fail)
- [2.5 Superseded seeds](#25-superseded-seeds)
- [2.6 Pilot sizes, and the literature-strand charting gate for interpretive fields](#26-pilot-sizes-and-the-literature-strand-charting-gate-for-interpretive-fields)
- [2.7 Seed composition and seed recency are registered as determinants of the descriptive map](#27-seed-composition-and-seed-recency-are-registered-as-determinants-of-the-descriptive-map)
- [2.8 The preliminary corpus cross-check is removed](#28-the-preliminary-corpus-cross-check-is-removed)
- [2.9 Lens.org is dropped; OpenAlex becomes the sole index of record](#29-lensorg-is-dropped-openalex-becomes-the-sole-index-of-record)

**[Amended field text](#amended-field-text)** — 35 registration fields, in the order they appear in the OSF form:

  1. Type of review — guidelines list
  2. Background — final paragraph of the first section
  3. Review stages — stage 3
  4. Review stages — stage 6
  5. End date
  6. Primary research question 3
  7. Secondary research question 6
  8. Dependent variables — literature strand
  9. Dependent variables — organizational strand
  10. Databases
  11. Interfaces
  12. Query strings — route (2)
  13. Search strategy justification
  14. Software — citation-network searching
  15. Search validation procedure
  16. Used exclusion criteria
  17. Miscellaneous search strategy details
  18. Entities to extract — literature strand, item 5
  19. Entities to extract — organizational strand
  20. Extraction stages — stage (1) and item 3
  21. Extraction reliability — literature strand
  22. Screening stages — pilot screening and AI validation
  23. Screening reliability — Group 3
  24. Screening reliability — final paragraph of the organizational strand description
  25. Sampling and sample size — the stopping rule
  26. Extraction reliability — organizational strand
  27. Data validation
  28. Synthesis plan — Tier 1
  29. Synthesis plan — Tier 3
  30. Synthesis plan — third organizational component
  31. Synthesis plan — inductive coding
  32. Expectations — (c)
  33. Publication bias analyses
  34. Sensitivity analyses — literature strand, check (b)
  35. Miscellaneous screening details — AI-use disclosure

**[Attachments added or superseded](#attachments-added-or-superseded)** · **[Checklist before filing](#checklist-before-filing)** · **[References added by this update](#references-added-by-this-update)**
...
The End date field estimates the organizational strand at "~4–6 weeks", run in parallel with the literature strand, and derives a planned end date of 30 June 2027 from that figure together with the other stage durations.

The organizational strand protocol attached to the same registration estimates the same work at approximately 480 research-assistant hours and 135 lead-reviewer hours — on the attachment's own reckoning, about 1.5 to 1.7 semesters for two assistants working ten hours a week. The registration and its attachment therefore give figures for the same stage that differ by roughly a factor of five, and the planned end date rests on the smaller one.
...
The revised set comprises [8–10 works, listed here by full citation with a stable identifier where one exists]. No work in the set is among the 23 frozen seeds, and no work in the set may be added to the seed set at any point during the review. The set is distributed across public interest technology, civic technology and the "for good" tradition.
...
The set is recorded in an attachment to this update and frozen on filing. It is frozen at filing rather than at the start of the Search stage so that it is covered by the assurance given above, under which no stage of the protocol had been executed.

A pre-2000 foundational work is not required. Backward-chasing depth is better tested by including a work situated two or three citation hops from the seed set, where one can be identified, than by a work old enough that it cannot substantively engage traditions that did not yet exist and would be correctly excluded at screening if recovered.
...
This change applies to the categorical interpretive entities only. **The metadata gate is retained unchanged:** field-level accuracy of 95% or above remains a hard condition of proceeding to the full run for metadata fields, and a metadata field below it after revision is extracted by the human without AI assistance. Metadata extraction has a determinate right answer against which accuracy can be measured on twenty-five observations; interpretive coding does not, which is the whole basis of the distinction below.
...
**As revised.** The cross-check is removed. The preliminary corpus is set aside without exception, matching the treatment of the ad hoc organizational collection and removing the last inconsistency between "set aside and not carried forward" and its actual use.
...
## Software — citation-network searching
...
## Extraction stages — stage (1) and item 3

Both amendments to this field are given together so that it is edited once.
...
[Stage (2) items 1 and 2 are unchanged.]
...
## Sensitivity analyses — literature strand, check (b)
...
# ATTACHMENTS ADDED OR SUPERSEDED

- **Search validation set (new).** The 8–10 landmark works, with full citations and a stable identifier where one exists, none of them among the 23 seeds. Each work is recorded with the arm of the search it tests and with which part of the two-part pass criterion its non-recovery counts toward — the expansion criterion, the hand-searching criterion, or neither, the last applying to Arnstein (1969) alone. Frozen on filing.
- **Frozen seed set (superseded; original retained).** Twenty-one field-level corrections across twenty of the twenty-three records; seeds 4, 14 and 21 were not corrected. Two are factual (seed 22's venue label, its identifier having been correct; seed 13's third author, Lisa M. Chambers to Lauren M. Chambers); the remainder are formatting (BibTeX brace protection, an escaped ampersand, and ` and ` author separators). Four columns added — *Expansion identifier*, *Preprint / superseded*, *Forward-chase viability*, *Verified*. A *Revision log* sheet records every correction with its before and after, the date, the assurance that no stage had been executed, and three outstanding items: the OpenAlex work ID for seed 20, the per-seed forward and backward yields to be recorded at first run, and the check of whether the index merges seed 22's preprint with its published record. **The 23 works identified are unchanged; no seed has been added, removed or substituted.**
- **Organizational strand protocol workbook (superseded).** Codebook amended so the Group 3 framework-dimension fields are `who_participates`, `who_benefits`, `what_counts_as_social_good`, `who_decides` and `what_counts_as_relevant_technology`, all marked PILOT; `ecosystem_role` retained under functional role with a note that it does not enter the saturation criterion; the undetermined rule recorded against the stopping rule.
- **Literature charting form (superseded).** Amended to the same five dimensions, so that RQ3 and RQ6 compare the strands on a shared axis.

---

# CHECKLIST BEFORE FILING

- [ ] Fill the four date placeholders: the filing date in the header, the revised planned end date in item 1.8 and again in the End date field, and the seed-file revision date in Miscellaneous search strategy details. Then fill the validation-set citations.
- [ ] Search the full registration for "four framework" and "four dimensions" and confirm no surviving reference (note that "four named Google.org cohorts" is correct and should remain).
- [ ] Confirm the literature charting form and the organizational codebook name the same five dimensions in the same words.
- [ ] Confirm whether citationchaser 0.0.4 supports OpenAlex and can pass an API key; if not, script the expansion directly against the API and record that in Software.
- [ ] Decide between the live OpenAlex API and a dated snapshot, and record the choice with its date.
- [ ] Search the full registration for "Lens" and confirm no surviving reference; Lens.org has no remaining role.
- [ ] Confirm the Search validation procedure field no longer contains the preliminary-corpus cross-check paragraph.
...
- [ ] Set the revised planned end date in item 1.8 and in the End date field, and check whether the 12-month search expiration falls before the new date.
- [ ] Establish whether seed 6 (arXiv 2204.11461, 2022) has a published version, and record its status in the seed file's Preprint / superseded column before the Search stage.
